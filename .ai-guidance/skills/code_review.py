...
class CodeReviewSkill(Skill):
                ...
                issues.append(
                    {
                        ...
                        "severity": (
                            "高"
                            if pattern_name in ["hardcoded_secrets", "sql_injection"]
                            else "中"
                        ),
    ...
    def generate_recommendations(self, review_results):
        ...
        if issue_types.get("セキュリティ", 0) > 0:
            recommendations.append(
                "🔒 デプロイ前にセキュリティ脆弱性に対処してください"
            )

        if issue_types.get("パフォーマンス", 0) > 3:
            recommendations.append(
                "⚡ 効率向上のためパフォーマンス最適化を検討してください"
            )

        if issue_types.get("バグリスク", 0) > 0:
            recommendations.append(
                "🐛 ランタイム問題を防ぐため潜在的バグパターンをレビューしてください"
            )

        if issue_types.get("スタイル", 0) > 10:
            recommendations.append(
                "🎨 自動コードフォーマッターの実行を検討してください"
            )
    ...
    def meets_severity_threshold(self, issue_severity, min_severity):
        ...
        return severity_levels.get(issue_severity, 0) >= severity_levels.get(
            min_severity, 1
        )
