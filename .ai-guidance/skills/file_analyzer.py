...
class FileAnalyzerSkill(Skill):
    ...
    async def execute(self, file_paths: List[str] = None,
        ...
        try:
            # ファイル検索
            if not file_paths:
                file_paths = await self._discover_files(include_patterns, exclude_patterns)
            ...
            for file_path in file_paths:
                try:
                    ...
                    if analysis:
                        ...
                        total_metrics['total_lines'] += analysis['metrics']['lines_count']
                        total_metrics['total_size_bytes'] += analysis['metrics']['size_bytes']
                        total_metrics['issues_count'] += len(analysis['issues'])

                        if analysis['metrics']['complexity_score'] > self.max_complexity:
    ...
    async def _discover_files(self, include_patterns: List[str] = None,
        ...
        # 対象拡張子でファイル検索
        search_patterns = include_patterns or [f"**/*{ext}" for ext in self.supported_extensions.keys()]
        ...
        for file_path in all_files:
            if not any(self._matches_pattern(file_path, pattern) for pattern in exclude_list):
    ...
    def _calculate_metrics(self, content: str, language: str) -> Dict[str, Any]:
        ...
        metrics['code_density'] = (
            ...
            metrics['lines_count'] if metrics['lines_count'] > 0 else 0
        )

        metrics['comment_ratio'] = (
            metrics['comment_lines'] / metrics['lines_count']
            if metrics['lines_count'] > 0 else 0
        )
    ...
    def _calculate_complexity(self, content: str, language: str) -> int:
        ...
        control_keywords = {
            ...
            'JavaScript': ['if', 'for', 'while', 'try', 'catch', 'switch', 'case', '&&', '||'],
            'TypeScript': ['if', 'for', 'while', 'try', 'catch', 'switch', 'case', '&&', '||'],
            'Java': ['if', 'for', 'while', 'try', 'catch', 'switch', 'case', '&&', '||'],
    ...
    def _detect_issues(self, content: str, language: str, file_path: str) -> List[Dict[str, Any]]:
        """コード課題の検出"""
        ...
        for i, line in enumerate(lines, 1):
            ...
            # ハードコードされた値
            if language in ['Python', 'JavaScript', 'TypeScript'] and 'import' not in line:
                magic_numbers = re.findall(r'\b\d{2,}\b', line)
                if magic_numbers and not any(num in ['100', '200', '404', '500'] for num in magic_numbers):
                    issues.append({
                        'type': 'magic_number',
                        'severity': 'warning',
                        'line': i,
    ...
    def _detect_language_specific_issues(self, content: str, language: str) -> List[Dict[str, Any]]:
        """言語固有の課題検出"""
    ...
    def _generate_analysis_report(self, analysis_results: List[Dict[str, Any]],
                                 ...
                                 total_metrics: Dict[str, Any]) -> str:
        """分析レポート生成"""
        ...
        report += f"- **高複雑度ファイル**: {total_metrics['high_complexity_files']}\n\n"
        ...
        # 品質指標
        if analysis_results:
            avg_complexity = sum(r['metrics']['complexity_score'] for r in analysis_results) / len(analysis_results)
            avg_line_length = sum(r['metrics']['average_line_length'] for r in analysis_results) / len(analysis_results)
        ...
        if high_issue_files:
            report += "## 高課題ファイル\n\n"
            for file_analysis in sorted(high_issue_files, key=lambda x: len(x['issues']), reverse=True)[:10]:
                report += f"- **{file_analysis['file_path']}**: {len(file_analysis['issues'])} 課題\n"
            report += "\n"

        return report

    def _generate_summary(self, analysis_results: List[Dict[str, Any]],
                         total_metrics: Dict[str, Any]) -> str:
        """サマリー生成"""
