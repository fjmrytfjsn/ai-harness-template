...
class LoggingMiddleware(BaseMiddleware):
    ...
    async def before_agent(self, request: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        self.logger.info(f"エージェント処理開始 [{self.session_id}]",
    ...
    async def before_model(self, prompt: str, context: Any) -> str:
        ...
        self.logger.debug(f"モデル呼び出し開始 [{self.session_id}]",
                         extra={'structured_data': log_data})
    ...
    async def wrap_tool_call(self, call_func, tool_name: str, *args, **kwargs):
        ...
        self.logger.info(f"ツール実行開始: {tool_name} [{self.session_id}]",
                        extra={'structured_data': log_data})
        ...
        try:
            ...
            self.logger.info(f"ツール実行成功: {tool_name} ({duration:.2f}s) [{self.session_id}]",
            ...
            self.logger.error(f"ツール実行失敗: {tool_name} ({duration:.2f}s) - {e} [{self.session_id}]",
    ...
    async def after_model(self, response: str, context: Any) -> str:
        ...
        self.logger.info(f"モデル応答完了 ({model_duration:.2f}s) [{self.session_id}]",
                        extra={'structured_data': log_data})
    ...
    async def after_agent(self, result: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        for tool_call in self.tool_calls:
            ...
            if tool_name not in tool_stats:
                tool_stats[tool_name] = {'count': 0, 'total_duration': 0, 'success_count': 0}
        ...
        result['session_metrics'] = {
            ...
            'tool_success_rate': len(successful_tools) / len(self.tool_calls) if self.tool_calls else 1.0,
            'tool_statistics': tool_stats
        }
