...
class SecurityMiddleware(BaseMiddleware):
    ...
    def _setup_patterns(self):
        ...
        self.pii_patterns = [
            ...
            # APIキー・トークン
            (re.compile(r'(?i)(?:api[_-]?key|token|secret)[:\s=]+[A-Za-z0-9+/=]{20,}'), 'APIキー'),
        ]
        ...
        self.dangerous_commands = [
            ...
            (re.compile(r'DELETE\s+FROM.*WHERE\s+1\s*=\s*1', re.IGNORECASE), '全データ削除'),
    ...
    async def before_agent(self, request: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        if self.mask_pii and message:
            ...
            if pii_found:
                self._log_security_event('pii_detected', {
                    'pii_types': pii_found,
    ...
    async def wrap_tool_call(self, call_func, tool_name: str, *args, **kwargs):
        ...
        # 実行前ログ
        self._log_security_event('tool_call_attempt', {
            'tool_name': tool_name,
            'args_count': len(args),
        ...
        try:
            ...
            # 実行成功ログ
            self._log_security_event('tool_call_success', {
                'tool_name': tool_name
            })
            ...
            # 一般エラーログ
            self._log_security_event('tool_call_error', {
                'tool_name': tool_name,
    ...
    def _log_security_event(self, event_type: str, details: Dict[str, Any]):
        ...
        # 構造化ログとして出力
        self.log_info(f"セキュリティイベント: {event_type}",
