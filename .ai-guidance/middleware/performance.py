...
class PerformanceMiddleware(BaseMiddleware):
    ...
    async def wrap_model_call(
        self, model_call, prompt: str, context: Dict[str, Any]
    ) -> Any:
        """モデル呼び出しのラップと最適化"""
    ...
    async def wrap_tool_call(
        self, tool_call, tool_name: str, context: Dict[str, Any]
    ) -> Any:
        """ツール呼び出しの並列化と最適化"""
    ...
    async def after_agent(self, result: Any, context: Dict[str, Any]) -> Any:
        ...
        if self.enable_metrics_collection:
            ...
            if "_perf_start_time" in context:
                ...
                logger.info(
                    f"Performance summary for {request_id}: "
    ...
    def _can_parallelize_tool(self, tool_name: str) -> bool:
        ...
        # 並列実行危険なツール（排他制御が必要）
        exclusive_tools = {"file_write", "git_commit", "deployment", "database_write"}
